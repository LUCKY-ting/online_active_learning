...
void getDiscr_value(double *x, double *H_t, int d, int classNum, double *discr_val){
    int i, j;
    double product = innerproduct(x,x,d);
    ...
    for (i = 0; i < classNum; i++)
    {
        ...
        discr_val[i] = discr_val[i] / product;
    }
}
		...
		int nrhs, const mxArray *prhs[])
{
    ...
    double *data,*w, *s_t, *index, *H_t, *sum_g_t, *N_t, *x, *errNum;
    double delta, eta, b, firstMax_value, secondMax_value, margin,value, pred_y, cur_loss, q_t, discr_winner, discr_runner, product;
    int i,j,k,p,N,d,classNum,y,winner, runner,runnerup, Z_t,low,high, nonzerosNum;
    mwIndex *ir, *jc;
    int * idx;
    /*Read Input Data*/
...
//     printf("Number of instances= %d, dim = %d, classNum = %d\n", N, d, classNum);
    ...
    sum_g_t = Malloc(double,(d-1)*classNum);
     ...
     for(i = 0; i < N; i++)
    {
        ...
        for (k = 0; k < classNum; k++){ //compute the discriminate value
            discr_value[k] = 0;
            for (p = 0; p < nonzerosNum; p++){
                discr_value[k] += pow(x[p],2) / H_t[k*(d-1) + idx[p]];
            }
        ...
        if(Z_t == 1){  // query the label y
            ...
            if (cur_loss > 0){
                for(k = 0; k < nonzerosNum; k++){
                   sum_g_t[runnerup * (d-1) + idx[k]] = sum_g_t[runnerup*(d-1) + idx[k]] + x[k]; 
                   s_t[runnerup * (d-1) + idx[k]] = sqrt(pow(s_t[runnerup*(d-1) + idx[k]], 2) + pow(x[k], 2));
                   H_t[runnerup * (d-1) + idx[k]] = delta + s_t[runnerup*(d-1) + idx[k]];
                   w[runnerup*(d-1)+ idx[k]] = -eta * (1/ H_t[runnerup*(d-1) + idx[k]] ) * sum_g_t[runnerup*(d-1) + idx[k]];
                     
                   sum_g_t[y * (d-1) + idx[k]] = sum_g_t[y*(d-1) + idx[k]] - x[k];
                   s_t[y * (d-1) + idx[k]] = sqrt(pow(s_t[y*(d-1) + idx[k]], 2) + pow(x[k], 2));
                   H_t[y * (d-1) + idx[k]] = delta + s_t[y*(d-1) + idx[k]];
                   w[y*(d-1)+ idx[k]] = -eta * (1/ H_t[y*(d-1) + idx[k]] ) * sum_g_t[y*(d-1) + idx[k]];
                } 
            }
        }  
        free(x);
