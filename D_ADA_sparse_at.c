        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    double *data,*w, *x, *s_t, *index, *H_t, *sum_g_t, *N_t, *accRate, *f1score;
        ...
        if(Z_t == 1){  // query the label y
            if (y * pred_v < 1){
                for(k = 0; k < nonzerosNum; k++){
                    sum_g_t[idx[k]] = sum_g_t[idx[k]] - y * x[k];
                    s_t[idx[k]] = sqrt(pow(s_t[idx[k]], 2) + pow(x[k], 2));
                    H_t[idx[k]] = delta + s_t[idx[k]];
                    w[idx[k]] = -eta * (1/ H_t[idx[k]] ) * sum_g_t[idx[k]];
                }
            }
        }
        free(x);
